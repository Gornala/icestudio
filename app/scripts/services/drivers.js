      ...
      function returnBrewPath() {
        ...
        try {
          ...
          if (
            ...
            fs.appendFileSync(
              common.LOGFILE,
              'Found homebrew in path: ' + brewPath + '\n'
            );
          }
        } catch (error) {
          if (
            ...
            fs.appendFileSync(
              common.LOGFILE,
              'Error detecting brew path:' + '\n'
            );
            fs.appendFileSync(common.LOGFILE, error + '\n');
          }
        }

        const arch = nodeChildProcess
          .execSync('uname -a')
          .toString()
          .trim()
          .toLowerCase();
      ...
      function enableDarwinDrivers(brewPackages, profileSetting) {
        ...
        try {
          ...
          if (
            ...
            fs.appendFileSync(
              common.LOGFILE,
              'Error installing driver: ' + error + '\n'
            );
          }
          alertify.error(
            gettextCatalog.getString('Error installing driver:') + '\n' + error
          );
          utils.endBlockingTask();
        }
      }
    ...
    `;
      ...
      function enableWindowsDrivers(type) {
        if (iceStudio.toolchain.apio && iceStudio.toolchain.apio < '0.9.6') {
          ...
          utils.beginBlockingTask();
          nodeSudo.exec(
            [common.APIO_CMD, 'drivers', `install ${type}`].join(' '),
            { name: 'Icestudio' },
            function (error, stdout, stderr) {
              utils.endBlockingTask();
              if (stderr) {
                alertify.error(
                  gettextCatalog.getString(
                    'Error enabling driver type {{type}}',
                    { type: type }
                  ),
                  30
                );
